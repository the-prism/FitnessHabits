...
public class ActiviteViewModel extends ViewModel {
    ...
    private LiveData<List<ActiviteEntry>> activiteList;
    private LiveData<List<ActiviteData>> activiteCategory;
    ...
    private void subscribeToDbChanges() {
        ...
        listActi = Transformations.map(activiteList,
                new Function<List<ActiviteEntry>, List<String>>() {
                    @Override
                    public List<String> apply(List<ActiviteEntry> listOfActivities) {
                        ArrayList<String> list = new ArrayList<>();
                        for (ActiviteEntry act : listOfActivities) {
                            list.add(act.getDate() + " " + act.getCategoryId());
                        }
                        return list;
                    }
                });
    ...
    public LiveData<List<ActiviteEntry>> getAllActiviteData() {
        return activiteList;
    }
    ...
    public void insertItem(ActiviteEntry activite) {
    ...
    public void createDb() {
        ...
        ActiviteEntry test = new ActiviteEntry();
        test.setDate("NOW");
        test.setIntensite(10);
        test.setCategoryId(1);
        repository.saveActiviteData(test);
        ActiviteEntry test2 = new ActiviteEntry();
